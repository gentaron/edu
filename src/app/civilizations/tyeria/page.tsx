    ...
    <div className="relative min-h-screen bg-edu-bg">
      ...
      <div className="relative z-10">
        <PageHeader
          ...
          wikiHref={`/wiki/${encodeURIComponent("ティエリア")}`}
        />
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6 border border-rose-400/20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-4">
                  <div className="flex gap-3"><span className="text-rose-400 font-medium shrink-0 w-20">指導者</span><Link href={`/wiki/${encodeURIComponent("グレイモンド・ハウザー")}`} className="text-edu-muted hover:text-rose-400 hover:underline">総帥グレイモンド・ハウザー</Link></div>
                ...
                <p className="text-sm text-edu-muted leading-relaxed">
                  ティエリアは宇宙最強の軍事力を誇る文明圏。軍事技術の輸出を経済基盤とし、その防衛ネットワークは宇宙随一と言われる。<Link href={`/wiki/${encodeURIComponent("グレイモンド・ハウザー")}`} className="text-edu-accent hover:underline">グレイモンド・ハウザー総帥</Link>の下、「軍事力なくして平和は守れない」という信条に基づき、強大な軍備を維持している。
                </p>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <p className="text-sm text-edu-muted leading-relaxed">
                  古くから軍事技術の研究に注力し、他文明圏に対する軍事技術輸出で独自の経済基盤を確立してきた。<Link href="/civilizations/granbell" className="text-amber-400 hover:underline">グランベル</Link>の経済的支配が進む中、ティエリアは軍事力で均衡を保つ政策をとってきた。この経済大国vs軍事大国の構造は、<Link href={`/wiki/${encodeURIComponent("ディオクレニス")}`} className="text-edu-accent hover:underline">ディオクレニス</Link>の<Link href={`/wiki/${encodeURIComponent("ネイサン・コリンド")}`} className="text-edu-accent hover:underline">ネイサン</Link>によって「<Link href={`/wiki/${encodeURIComponent("トゥキディデスの罠")}`} className="text-edu-accent hover:underline">トゥキディデスの罠</Link>」として指摘されている。
                </p>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <p className="text-sm text-edu-muted leading-relaxed">
                  グランベルの経済的支配に対し、軍事力で均衡を図る政策を継続。<Link href="/civilizations/elyseon" className="text-emerald-400 hover:underline">エレシオン</Link>の<Link href={`/wiki/${encodeURIComponent("女王リアナ・ソリス")}`} className="text-edu-accent hover:underline">リアナ女王</Link>とは軍拡を巡って対立しており、<Link href={`/wiki/${encodeURIComponent("宇宙連合会合")}`} className="text-edu-accent hover:underline">宇宙連合会合</Link>でもこの対立が表面化した。ファルージャの調停試みに対しては懐疑的な態度をとっている。
                </p>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <Link href={`/wiki/${encodeURIComponent("グレイモンド・ハウザー")}`} className="block edu-card rounded-lg p-4 border border-rose-400/20 hover:border-rose-400/40 transition-colors">

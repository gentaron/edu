          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <RevealSection>
              <div className="edu-card rounded-xl p-6 border border-violet-400/20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-4">
                  <div className="flex gap-3"><span className="text-violet-400 font-medium shrink-0 w-20">指導者</span><Link href={`/wiki/${encodeURIComponent("マドリス・カーネル")}`} className="text-edu-muted hover:text-violet-400 hover:underline">評議会代表マドリス・カーネル</Link></div>
                  <div className="flex gap-3"><span className="text-violet-400 font-medium shrink-0 w-20">専門</span><span className="text-edu-muted">文化的影響力・外交</span></div>
                  <div className="flex gap-3"><span className="text-violet-400 font-medium shrink-0 w-20">理念</span><span className="text-edu-muted">「文化の力が宇宙全体を結びつける鍵」</span></div>
                  <div className="flex gap-3"><span className="text-violet-400 font-medium shrink-0 w-20">ランキング</span><span className="text-edu-muted">宇宙第4位</span></div>
                </div>
                <p className="text-sm text-edu-muted leading-relaxed">
                  ファルージャは文化的影響力と外交力で宇宙に君臨する文明圏。対立する勢力間の調停役として重要な地位を占め、<Link href={`/wiki/${encodeURIComponent("マドリス・カーネル")}`} className="text-edu-accent hover:underline">マドリス・カーネル</Link>代表の下、「文化の力が宇宙全体を結びつける鍵」という理念を掲げている。
                </p>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <Link href={`/wiki/${encodeURIComponent("マドリス・カーネル")}`} className="block edu-card rounded-lg p-4 border border-violet-400/20 hover:border-violet-400/40 transition-colors">
                  <h3 className="text-sm font-bold text-violet-400">評議会代表マドリス・カーネル</h3>
                  <p className="text-xs text-edu-muted mt-1">ファルージャ評議会代表。「文化の力が宇宙全体を結びつける鍵」と信じる調停者。</p>
                </Link>
              </div>
            </RevealSection>
            ...
            <Link href="/civilizations" className="inline-flex items-center gap-1 text-xs text-edu-muted hover:text-violet-400 transition-colors">
              ← 文明圏一覧に戻る
            </Link>
          </div>
        </main>

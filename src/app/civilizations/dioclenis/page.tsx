    ...
    <div className="relative min-h-screen bg-edu-bg">
      ...
      <div className="relative z-10">
        <PageHeader
          ...
          wikiHref={`/wiki/${encodeURIComponent("ディオクレニス")}`}
        />
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <RevealSection>
              <div className="edu-card rounded-xl p-6 border border-cyan-400/20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-4">
                  <div className="flex gap-3"><span className="text-cyan-400 font-medium shrink-0 w-20">指導者</span><Link href={`/wiki/${encodeURIComponent("ネイサン・コリンド")}`} className="text-edu-muted hover:text-cyan-400 hover:underline">科学宰相ネイサン・コリンド</Link></div>
                ...
                <p className="text-sm text-edu-muted leading-relaxed">
                  ディオクレニスは宇宙探査と科学技術研究の最前線に立つ文明圏。<Link href={`/wiki/${encodeURIComponent("ネイサン・コリンド")}`} className="text-edu-accent hover:underline">ネイサン・コリンド</Link>科学宰相の下、「<Link href={`/wiki/${encodeURIComponent("トゥキディデスの罠")}`} className="text-edu-accent hover:underline">トゥキディデスの罠</Link>」を提起し、宇宙共同探査プロジェクトを提案するなど、先見性のある政策を展開している。
                </p>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <p className="text-sm text-edu-muted leading-relaxed">
                  科学技術の研究開発に特化し、宇宙探査の分野で多大な成果を挙げてきた。ネイサン・コリンドの下、<Link href={`/wiki/${encodeURIComponent("宇宙連合会合")}`} className="text-edu-accent hover:underline">宇宙連合会合</Link>で4つの具体的提案を行った。
                </p>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <div className="p-4 rounded-lg bg-red-500/5 border border-red-500/20 mb-4">
                  <p className="text-sm text-edu-muted leading-relaxed">
                    <span className="text-red-400 font-medium">警告:</span> <Link href={`/wiki/${encodeURIComponent("ネイサン・コリンド")}`} className="text-edu-accent hover:underline">ネイサン</Link>は、<Link href="/civilizations/granbell" className="text-amber-400 hover:underline">グランベル</Link>（経済1位）と<Link href="/civilizations/tyeria" className="text-rose-400 hover:underline">ティエリア</Link>（軍事3位）の間に「<Link href={`/wiki/${encodeURIComponent("トゥキディデスの罠")}`} className="text-edu-accent hover:underline">トゥキディデスの罠</Link>」が成立する危険性を指摘。かつての<Link href={`/wiki/${encodeURIComponent("アポロン・Dominion大戦")}`} className="text-edu-accent hover:underline">アポロン文明圏とDominionの関係</Link>も同様の構造だったと分析している。
                  </p>
                </div>
              </div>
            </RevealSection>
            ...
            <RevealSection>
              <div className="edu-card rounded-xl p-6">
                ...
                <Link href={`/wiki/${encodeURIComponent("ネイサン・コリンド")}`} className="block edu-card rounded-lg p-4 border border-cyan-400/20 hover:border-cyan-400/40 transition-colors">

        ...
        <div className="mx-auto max-w-5xl px-4 py-8 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-wide text-edu-text">
              {tl("AI 音楽生成", "AI Music Generation", lang)}
            </h1>
            <p className="mt-1 text-sm text-edu-muted">
              {tl(
                "ACE-Step 1.5 を使った高品質なAI音楽生成",
                "High-quality AI music generation powered by ACE-Step 1.5",
                lang
              )}
            </p>
          </div>
          ...
          {(["create", "library"] as Tab[]).map((t) => (
            <button
              ...
              className={`flex-1 rounded-md py-2 text-sm font-medium transition-colors ${
                tab === t ? "bg-edu-accent text-edu-bg" : "text-edu-text-dim hover:text-edu-text"
              }`}
            >
              {t === "create" ? tl("生成", "Create", lang) : tl("ライブラリ", "Library", lang)}
            </button>
          ))}
        </div>

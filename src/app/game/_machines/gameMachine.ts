// @ts-nocheck — legacy xstate v5 machine, types not fully aligned
    ...
    draw: {
      entry: assign(({ context }) => {
        ...
        return {
          ...
          log: [
            ...context.log,
            `ターン${context.turnNumber}: ${cp === "p1" ? "先攻" : "後攻"}ドロー`,
          ],
        }
      }),
      always: "main",
        ...
        ATTACK_SLOT: {
          actions: assign(({ context, event }) => {
            ...
            return {
              ...
              phase: "main" as const,
              selectedAttacker: null,
              log: [...context.log, `${attacker.card.name} → ${defender.card.name} 戦闘`],
              winner: w,
            }
          }),
        },

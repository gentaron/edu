// @ts-nocheck — legacy game demo page
"use client"
...
const ALL_DEMO_CARDS: CardDef[] = [
  ...
  },
  {
    ...
    cost: 6,
    power: 8,
    description: "Valoria連合圏を主導する現役最強格。",
  },
  {
    id: "c3",
    name: "レイラ",
    ...
    cost: 4,
    power: 6,
    description: "Pink Voltage。AURALIS第二世代。",
  },
  {
    id: "c4",
    name: "ケイト",
    ...
    cost: 3,
    power: 4,
    description: "AURALIS Collective創設者。",
  },
  {
    id: "c5",
    name: "アルファ・ケイン",
  ...
  },
  {
    ...
    cost: 3,
    power: 3,
    description: "AURALIS Collectiveに関わる人物。",
  },
  {
    id: "c10",
    name: "ガロ",
    ...
    return (
      <div className="min-h-screen bg-cosmic-dark flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/game" className="text-sm text-electric-blue hover:underline">
            ← デッキ選択に戻る
          </Link>
        </div>
      </div>
    ...
    <div className="relative min-h-screen bg-cosmic-dark flex flex-col">
      ...
      <nav className="shrink-0 glass-card border-b border-cosmic-border/50 z-50">
        <div className="max-w-6xl mx-auto px-4 flex items-center gap-4 h-12">
          <Link
            href="/game"
            className="text-xs text-cosmic-muted hover:text-electric-blue transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-cosmic-gradient">
              ターン{ctx.turnNumber} —{" "}
              {isFinished ? "終了" : isPlayerTurn ? "あなたのターン" : "相手のターン"}
            </span>
          </div>
      ...
      <div className="flex-1 flex flex-col gap-3 p-3 max-w-6xl mx-auto w-full overflow-y-auto">
        ...
        <Board
          ...
          onSlotClick={isAttackPhase ? handleSlotClick : undefined}
        />
        ...
        <div className="flex gap-1.5 justify-center">
          {ctx.p2.hand.map((_, i) => (
            <div
              key={i}
              className="w-14 h-20 rounded-lg border border-cosmic-border/40 bg-cosmic-surface/50"
            />
          ))}
        </div>
        ...
        <div className="flex items-center justify-center gap-3 shrink-0 py-2">
          {isAttackPhase && (
            <>
              <button
                onClick={() => send({ type: "ATTACK_PLAYER" })}
                className="px-3 py-1.5 text-xs font-bold rounded-lg bg-red-500/20 border border-red-500/40 text-red-400 hover:bg-red-500/30 transition-all flex items-center gap-1"
              >
                <Target className="w-3 h-3" /> 直接攻撃
              </button>
              <button
                onClick={() => send({ type: "DESELECT" })}
                className="px-3 py-1.5 text-xs font-bold rounded-lg bg-cosmic-surface border border-cosmic-border text-cosmic-muted hover:text-cosmic-text transition-all"
              >
                解除
              </button>
            </>
          ...
          {isPlayerTurn && ctx.phase === "main" && (
            <button
              ...
              className="px-4 py-2 text-xs font-bold rounded-lg bg-nebula-purple/20 border border-nebula-purple/40 text-nebula-purple hover:bg-nebula-purple/30 transition-all flex items-center gap-1"
            >
          ...
          {isFinished && (
            <div className="text-center">
              ...
              <Link href="/game" className="text-xs text-electric-blue hover:underline">
                ← 再戦する
              </Link>
            </div>
          )}
        ...
        <Board
          player={ctx.p1}
          label="あなたの場"
          selectedAttacker={ctx.selectedAttacker ?? null}
        ...
        <div className="max-w-6xl mx-auto px-4 py-2 max-h-24 overflow-y-auto">
          <div className="flex gap-1.5 flex-wrap">
            {ctx.log.slice(-20).map((entry, i) => (
              <span
                key={i}
                className="text-[10px] text-cosmic-muted bg-cosmic-dark/50 px-2 py-0.5 rounded"
              >
                {entry}
              </span>
            ))}

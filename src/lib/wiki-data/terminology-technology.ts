...
export const WIKI_TECHNOLOGY: WikiEntry[] = [
  /* 技術・概念 */
  {
    ...
    subCategory: "技術・概念",
    description:
      "ブラックホールの事象の地平面近傍で観測される特異な物理現象を応用した高次元空間アクセス技術であり、一般相対性理論のシュヴァルツシルト解における地平面の数学的構造を人為的に再現することで、三次元時空の局所的な曲率を操作する。この技術は超弦理論のCalabi-Yau多様体がコンパクト化された六つの余分な次元と物理的時空との境界領域に干渉し、空間ホールと呼ばれる安定した次元間ポータルの構築を支える理論的根幹となっている。元々は惑星ビブリオのロレンツィオ国際大学で純粋物理学の研究として始まったが、AdS/CFT対応と呼ばれる反ド・ジッター空間と共形場理論の双対性を応用した計算モデルにより実用化が加速した。ZAMLT期には国家機密として独占され、テクノ文化ルネサンス期以降に民用化が進んだが、シルバー・ヴェノムやアルファ・ヴェノムによる武器化が深刻な問題となっている。次元ピラミッド理論はこの技術の体系化された理論框架であり、E16文明圏における最重要の基盤技術の一つとして位置づけられている。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "次元極地平技術を応用して構築された安定した次元間ポータルで、アインシュタイン・ローゼン橋の数学的構造を人為的に制御し、二つの遠く離れた時空領域を位相的に接続する。空間ホールは一般相対性理論におけるワームホール解を安定化するために、負のエネルギー密度を持つカシミール効果を応用したエキゾチック物質のリングを入口に配置し、輻射崩壊を防ぐ構造となっている。テクノ文化ルネサンス期以前は軍事・企業エリートのみが利用できる限定された技術だったが、技術の民主化に伴い一般市民も利用可能となり、E16連星系内の社会的・経済的統合を劇的に進展させた。空間ホールの安定性はSSレンジの通信技術によって量子誤り訂正符号を適用して制御されており、過度な利用は周囲の時空の曲率テンソルに歪みを蓄積するリスクがあるため、UECOの管理下でスループットが規制されている。シルバー・ヴェノムやアルファ・ヴェノムは空間ホールを軍事的に利用し、奇襲攻撃や撤退に活用している。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "Eros-7を原生地とする特殊な生物群で、他の生命体から生体エネルギーを吸収して生存・増殖する生態的特徴を持つ。搾取生物の原種はEros-7の独自の生態系の中で自然進化したものであり、通常状態では他種との接触を避ける習性を持つ穏やかな生物であった。しかしE380年頃に発生した遺伝子変異により、一部の個体が攻撃性を獲得し、他の生命体の生体エネルギーをも吸収する能力を身につけた。この変異は生体分子レベルの量子トンネル効果を利用したエネルギー伝達機構の異常亢進によるものと推測され、変異種の爆発的な増殖がスライム危機を引き起こし、E16連星系全体に大規模な被害をもたらした。搾取生物の生態学研究はEros-7のマトリカル社会によって最も進んでおり、彼女たちが独自に開発した生物学的封じ込め技術が危機収束の鍵となった。現在、搾取生物の制御はEros-7の特別管理下に置かれており、その研究から生まれた医療技術は再生医療や生体エネルギー工学に応用されている。この生物群は災害の源泉であると同時に、新たな医療技術の可能性を秘めた存在として再評価されている。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "リミナル・フォージが開発した画期的な放送方式であり、量子もつれの非局所性と一般相対性理論の因果構造を統合して、時空間の次元を超えて情報を送信する。特にE528年には、E528年の芸術作品を地球AD2026年のインターネット上に放送するという前例のないクロス次元放送を実現し、二つの時代間の文化交流の架け橋となった。この技術の理論的基盤はテミルタロンのサイケデリック・コスモロジーに由来し、ホログラフィック原理の枠組みで次元極地平の性質を応用したものである。情報の時間的伝達における因果律の保存は、Shannonエントロピーの時間非対称性を利用したエントロピー補償プロトコルによって保証されており、送信側と受信側の間で情報エントロピーの収支を均衡させる仕組みが組み込まれている。時相放送の最大の技術的課題は、量子デコヒーレンスによる信号劣化の抑制と、クローズド・タイムライク・カーブを回避しつつ情報を時間軸に沿って転送する数学的構造の維持である。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "ペルセポネ仮想宇宙を構成する最も重要な基盤技術であり、ホログラフィック原理を工学的に実装した量子情報投影システムである。クオリア・コアによって生成される感情データと感覚情報を、AdS/CFT対応に基づく境界上の量子場の配列として符号化し、仮想空間上に物理現実として投影する。視覚・聴覚・触覚だけでなく、喜怒哀楽などの感情的な体験までも、量子エンタングルメントを用いた多点相関関数の再構成によって忠実に再現することができる。プライマリー・フィールドは境界上の情報が内部のバルク時空を完全に記述するというホログラフィック原理の直接的な応用であり、仮想空間内の物理的振る舞いは境界の量子情報の変化に従って動的に更新される。この技術によりペルセポネは単なる仮想空間を超えた「第二の現実」として機能し、移民者たちが過酷な現実環境から一時的に解放される場となっている。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "搾取生物の最も標準的で広く量産された形態であり、形状記憶ポリマーと液晶エラストマーの複合材料に類似した柔軟な触手構造を持つ。対象に巻き付いて生体エネルギーを吸収する機能は、触手表面のナノスケールのパターンが生体膜の脂質二重層と融合し、濃度勾配を駆動力としたエネルギーの熱力学的輸送を行うことに基づいている。ZAMLTとEros-7の搾取施設で大量に生産され、労働者の監視や逃亡防止、エネルギー採取の補助作業など多目的に使用された。触手の先端には微細な神経接続端子があり、接触対象の生体電気信号をリアルタイムで監視することができる。スライム危機後は大部分が破棄されたが、一部は解体・研究され、その生体エネルギー伝達機構はナノメディシンのエネルギー供給技術に応用されている。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "アヤカ・リンが独自に開発した個人用防御技術であり、カシミール効果の負のエネルギー密度を利用した極薄のエネルギーフィルムを体表に展開する。このフィルムは量子真空の零点振動を局所的に操作し、搾取生物の物理攻撃やエネルギー攻撃を効果的に遮断する。フィルムの最小展開時の形状が水着に類似していることからビキニバリアと命名された。防御範囲と強度はアヤカの集中力に依存し、量子ゼロ点エネルギーの抽出効率が精神状態と相関する量子力学的心身相関に基づいている。最大展開時には周囲数メートルの範囲をカバーでき、カウパー波による攻撃動作との組み合わせにより、防御と反撃を同時に行う独自の戦闘スタイルを可能にした。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "搾取生物のエネルギーを効率的に変換・蓄積する装置であり、生体熱力学の原理に基づいて設計されている。リリス・ヴェインがその設計・開発を主導し、搾取生物が吸収した生体エネルギーを電気化学反応と熱電変換を通じて電力や次元エネルギーに変換する仕組みを持つ。バイオリアクターの核心は、細胞呼吸の電子伝達鎖から抽出されたエネルギーを、超伝導コイルを用いた磁気エネルギー貯蔵システムに効率的に変換する点にある。ZAMLTとEros-7の搾取体制におけるエネルギー供給の要であり、スクイーズ・アビスなど大規模な搾取施設の心臓部として機能した。シャドウ・ユニオンのナノハッキング技術による妨害活動の主な標的となり、Eros-7のエネルギーインフラの脆弱性を露呈した。",
  },
...

  {
    ...
    subCategory: "技術・概念",
    description:
      "テレパシーや念動力、エネルギー操作といった超常的な精神能力の総称であり、脳内のニューロン活動が量子コヒーレントな状態を維持し、外部環境の物理的状態に直接干渉できるという仮説に基づいている。シルヴィア・クロウは特に強力なエスパー能力の持ち主であり、Eros-7で発生した搾取生物危機の収束に決定的な役割を果たした。彼女は能力を用いて暴走した搾取生物の生体電気信号に干渉し、制御を試みて危機の沈静化に貢献した。エスパー能力の発現には先天的な素質と後天的な訓練の両方が関与するとされ、脳の特定領域における量子トンネル効果の増幅がその物理的基盤と推測されている。ヒーローエージェンシーでは能力者の早期発見と育成プログラムが実施されている。能力の強度には個人差が大きく、一部の高能力者は都市規模の影響力を持つとされる。",
  },
  ...
  },
  {
    ...
    subCategory: "技術・概念",
    description:
      "宇宙空間における高エネルギー宇宙放射線（太陽フレア粒子、銀河宇宙線など）から人命と電子機器を保護する技術体系である。受動的防護では水、ポリエチレン、アルミニウムなどの材料の質量厚によって放射線を減衰させ、能動的防護では超伝導磁石による強力な磁場を生成して荷電粒子をローレンツ力で偏向させる。E16文明圏ではナノセル・インプラントの放射線修復機能と組み合わせた多層的な防護システムが採用されている。宇宙放射線の中でも特に高いエネルギーを持つ重イオン（HZE粒子）は、材料中でのカスケード的な二次粒子生成を引き起こし、電子機器のシングルイベント・アップセット（SEU）の主要原因となるため、量子エラー訂正符号を用いた放射線耐性電子回路の開発が進められている。",
  },
  {
  ...
  },
  {
    ...
    subCategory: "技術・概念",
    description:
      "光速未満の速度で恒星間空間を航行する輸送システムであり、特殊相対性理論のローレンツ因子 γ=1/√(1-v²/c²) に従い、光速に近づくにつれて時間遅延効果が顕著になる。亜光速星間輸送船は反物質エンジンまたは核融合パルス推進を主推進力として採用し、長期間の連続加速と減速により目的地恒星系に到達する。船内時間は地球時間に比べて大幅に短縮されるため、乗員は相対論的効果による時間遅延の恩恵を受けるが、出発時の地球との通信には片道の航行年数に等しい時間的遅れが生じる。E16文明圏では曲率航法や空間ホールが利用できない遠距離の恒星間航行において亜光速輸送が補完的な役割を果たしており、人工冬眠システムと組み合わせた数十年規模の長期航行が実施されている。",
  },

...
import { BinaryWriter, BinaryReader, BinaryEncoder, BinaryIndex, crc32 } from '@/metal/binary-protocol'

describe('BinaryWriter — encode throughput', () => {
  bench('writeVarInt — single value (0..300)', () => {
    const w = new BinaryWriter(64)
    for (let i = 0; i < 300; i++) { w.writeVarInt(i) }
  })

  bench('writeString — Japanese text (100 chars)', () => {
    const w = new BinaryWriter(512)
    const text = '永遠の支配宇宙エデュは500年以上にわたる壮大な歴史を持つSF宇宙設定プロジェクトである。'
    for (let i = 0; i < 50; i++) { w.writeString(text) }
  })
  ...
  bench('writeValue — nested object (10 fields)', () => {
    ...
    for (let i = 0; i < 100; i++) { w.writeValue(obj) }
    void w.toUint8Array()
  })
  ...
  bench('writeValue — complex nested array+object structure', () => {
    ...
    for (let i = 0; i < 50; i++) { w.writeValue(complex) }
    void w.toUint8Array()
  })
})
...
describe('BinaryReader — decode throughput', () => {
  ...
  bench('prepare buffer', () => {
    const w = new BinaryWriter(2048)
    for (let i = 0; i < 500; i++) { w.writeVarInt(i) }
    for (let i = 0; i < 100; i++) { w.writeString('テスト文字列でパフォーマンスを計測する') }
    for (let i = 0; i < 50; i++) { w.writeI32(i * 100); w.writeF64(i * 3.14) }
    buffer = w.toBuffer()
  }, { iterations: 1 })

  bench('readVarInt — 500 sequential values', () => {
    const r = new BinaryReader(buffer)
    for (let i = 0; i < 500; i++) { r.readVarInt() }
  })

  bench('readString — 100 Japanese strings', () => {
    const r = new BinaryReader(buffer)
    for (let i = 0; i < 500; i++) { r.readVarInt() } // skip varints
    for (let i = 0; i < 100; i++) { r.readString() }
  })
})
...
describe('CRC32 throughput', () => {
  ...
  bench('crc32 — 10KB payload', () => {
    ...
    for (let i = 0; i < 10_240; i++) { data[i] = i & 255 }
    void crc32(data)
  })

  bench('crc32 — 100KB payload', () => {
    const data = new Uint8Array(102_400)
    for (let i = 0; i < 102_400; i++) { data[i] = i & 255 }
    void crc32(data)
  })
})

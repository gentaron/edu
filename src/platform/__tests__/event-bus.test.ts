...
describe("EventBus", () => {
  ...
  describe("subscribe/publish", () => {
    it("receives battle:start event", () => {
      ...
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
    ...
    it("receives battle:ability event", () => {
      let received = false
      eventBus.subscribe("battle:ability", () => { received = true })
      eventBus.publish({ type: "battle:ability", ability: "attack", characterId: "c1" })
      expect(received).toBe(true)
    })

    it("receives battle:turn event", () => {
      let received = false
      eventBus.subscribe("battle:turn", () => { received = true })
    ...
    it("receives battle:victory event", () => {
      ...
      eventBus.subscribe("battle:victory", (e) => {
        received = true
        expect(e.enemyId).toBe("void-king")
        expect(e.turnsUsed).toBe(10)
      })
      eventBus.publish({ type: "battle:victory", enemyId: "void-king", turnsUsed: 10 })
      expect(received).toBe(true)
    })

    it("receives battle:defeat event", () => {
      let received = false
      eventBus.subscribe("battle:defeat", () => { received = true })
      eventBus.publish({ type: "battle:defeat", enemyId: "test", turnsUsed: 5 })
      expect(received).toBe(true)
    })

    it("receives battle:reset event", () => {
      let received = false
      eventBus.subscribe("battle:reset", () => { received = true })
    ...
    it("receives deck:card-added event", () => {
      ...
      eventBus.publish({ type: "deck:card-added", cardId: "c1", deckSize: 1 })
      expect(received).toBe(true)
    })

    it("receives deck:card-removed event", () => {
      let received = false
      eventBus.subscribe("deck:card-removed", () => { received = true })
      eventBus.publish({ type: "deck:card-removed", cardId: "c1", deckSize: 4 })
      expect(received).toBe(true)
    })

    it("receives deck:cleared event", () => {
      let received = false
      eventBus.subscribe("deck:cleared", () => { received = true })
    ...
    it("receives wiki:entry-viewed event", () => {
      ...
      eventBus.subscribe("wiki:entry-viewed", (e) => {
        received = true
        expect(e.entryId).toBe("Diana")
      })
      eventBus.publish({ type: "wiki:entry-viewed", entryId: "Diana" })
      expect(received).toBe(true)
    })
    ...
    it("receives story:opened event", () => {
      ...
      eventBus.subscribe("story:opened", (e) => {
        received = true
        expect(e.slug).toBe("diana-world")
      })
      eventBus.publish({ type: "story:opened", slug: "diana-world" })
      expect(received).toBe(true)
    })

    it("receives ui:theme-toggled event", () => {
      let received = false
      eventBus.subscribe("ui:theme-toggled", () => { received = true })
  ...
  describe("multiple subscribers", () => {
    it("notifies multiple subscribers of same event", () => {
      let count1 = 0
      let count2 = 0
      eventBus.subscribe("battle:start", () => { count1++ })
      eventBus.subscribe("battle:start", () => { count2++ })
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
  ...
  describe("unsubscribe", () => {
    it("unsubscribes with returned function", () => {
      let received = 0
      const unsub = eventBus.subscribe("battle:start", () => { received++ })
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
    ...
    it("does not affect other subscribers", () => {
      let received1 = 0
      let received2 = 0
      const unsub = eventBus.subscribe("battle:start", () => { received1++ })
      eventBus.subscribe("battle:start", () => { received2++ })
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
  ...
  describe("subscribeAll", () => {
    it("receives all event types", () => {
      ...
      eventBus.subscribeAll((e) => { received.push(e.type) })
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
      eventBus.publish({ type: "deck:card-added", cardId: "c1", deckSize: 1 })
      eventBus.publish({ type: "wiki:entry-viewed", entryId: "Diana" })
    ...
    it("subscribeAll can be unsubscribed", () => {
      let received = 0
      const unsub = eventBus.subscribeAll(() => { received++ })
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
      expect(received).toBe(1)
      unsub()
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
  ...
  describe("clear", () => {
    it("removes all subscribers", () => {
      let received = 0
      eventBus.subscribe("battle:start", () => { received++ })
      eventBus.subscribeAll(() => { received++ })
      eventBus.clear()
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })
  ...
  describe("event isolation", () => {
    it("does not notify subscribers of different event types", () => {
      ...
      eventBus.subscribe("battle:start", () => { battleReceived = true })
      eventBus.subscribe("deck:card-added", () => { deckReceived = true })
      eventBus.publish({ type: "battle:start", enemyId: "test", deckSize: 5 })

...
export default function WikiView({
  ...
  const activeCategoryConfig = selectedCategory
      ...
      <div className="min-h-screen bg-[#08080d] text-white">
        <div className="max-w-3xl mx-auto px-6 pt-20 pb-16">
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-white/90">
                {langProp === "en" && activeEntry.nameEn ? activeEntry.nameEn : activeEntry.name}
              </h1>
              {activeEntry.nameEn && langProp !== "en" && (
                <p className="text-sm text-white/40">{activeEntry.nameEn}</p>
              )}
            </div>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <span className="px-2.5 py-1 rounded-full border border-white/10 text-xs text-white/60">
              {activeEntry.category}
            </span>
            {activeEntry.subCategory && (
              <span className="px-2.5 py-1 rounded-full border border-white/10 text-xs text-white/60">
                {activeEntry.subCategory}
              </span>
            )}
            {activeEntry.tier && (
              <span className="px-2.5 py-1 rounded-full border border-amber-500/30 text-xs text-amber-400">
                {tlTier(activeEntry.tier, langProp)}
              </span>
            )}
            {activeEntry.era && (
              <span className="px-2.5 py-1 rounded-full border border-white/10 text-xs text-white/60">
          ...
          <div className="p-6 bg-white/[0.02] border border-white/5 rounded-lg mb-8">
            ...
            <p className="text-sm text-white/70 leading-relaxed">
              {langProp === "en" && activeEntry.descriptionEn
          ...
          {activeEntry.sourceLinks && activeEntry.sourceLinks.length > 0 && (
            <div className="p-6 bg-white/[0.02] border border-white/5 rounded-lg mb-8">
              <h2 className="text-[11px] text-white/30 mb-5 uppercase tracking-widest">
                {tl("参照リンク", "Source Links", langProp)}
              </h2>
              <div className="space-y-2">
                {activeEntry.sourceLinks.map((link, i) => (
                  <a
                    ...
                    className="flex items-center gap-2 px-4 py-3 rounded-lg border border-white/5 bg-white/[0.01] text-sm text-white/60 hover:text-white/80 hover:bg-white/[0.03] transition-all"
                  >
      ...
      <div className="max-w-3xl mx-auto px-6 pt-8 pb-6">
        <div className="flex items-center justify-between gap-4 mb-1">
          <h1 className="text-xl font-bold text-white/90">
            {tl("EDU Wiki 百科事典", "EDU Wiki Encyclopedia", langProp)}
          </h1>
          <LangToggle lang={langProp} setLang={setLang} />
        </div>
        ...
        {isSearching ? (
          <div>
            <p className="text-xs text-white/30 mb-4">
              {searchResults.length} {tl("件", " entries", langProp)}
            </p>
            {searchResults.length === 0 ? (
              <p className="text-xs text-white/30 text-center py-12">
                {tl("該当するエントリがありません", "No matching entries found", langProp)}
              </p>
            ) : (
              <div className="space-y-2">
                {searchResults.map((entry) => (
                  <button
                    ...
                    className={`block w-full text-left p-3 border-l-[3px] ${BORDER_COLORS[entry.category] || "border-l-white/10"} bg-white/[0.01] hover:bg-white/[0.03] border border-r border-b border-t border-white/5 hover:border-red-400/30 transition-colors`}
                  >
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-xs font-medium text-white/80">
                        {langProp === "en" && entry.nameEn ? entry.nameEn : entry.name}
                      </span>
                      {entry.nameEn && langProp !== "en" && (
                        <span className="text-[10px] text-white/30">{entry.nameEn}</span>
                      )}
                    </div>
                    <p className="text-[11px] text-white/30 leading-relaxed line-clamp-2">
                      {(langProp === "en" && entry.descriptionEn
                        ? entry.descriptionEn
                        : entry.description
                      ).slice(0, 100)}
                      …
          ...
          <div>
            ...
            >
              ...
              {tl("カテゴリ一覧に戻る", "Back to categories", langProp)}
            </button>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-sm font-bold text-white/80">
                {langProp === "en" ? activeCategoryConfig.labelEn : activeCategoryConfig.label}
              </h2>
              <span className="text-[10px] text-white/30">
                {categoryEntries.length} {tl("件", " entries", langProp)}
              </span>
            </div>
            <div className="space-y-2">
                  ...
                  <button onClick={() => setSelectedCategory(cat.key)} className="w-full text-left">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-white/80">
                          {langProp === "en" ? cat.labelEn : cat.label}
                        </h2>

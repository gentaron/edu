/* ═══════════════════════════════════════════
   L4 TRANSPORT — Battle State
   Zustand store hidden behind L4 API.
   Components should NOT import from zustand directly.
   ═══════════════════════════════════════════ */

import { create } from "zustand"
import type { GameCard, Enemy, AbilityType, FieldChar } from "@/types"
import { charMaxHp, calculateEffectDamage, calculateEnemyDamage } from "@/lib/battle-logic"
import { eventBus } from "../event-bus"

export type BattlePhase = "idle" | "playerTurn" | "resolving" | "enemyTurn" | "victory" | "defeat"
...
export interface BattleSnapshot {
  ...
  turn: number
  fieldCharacters: readonly FieldChar[]
...
interface BattleState extends BattleSnapshot {
...
function addLog(setter: (fn: (s: BattleState) => Partial<BattleState>) => void, msg: string) {
...
export const useBattleStore = create<BattleState>((set, get) => ({
  ...
  getSnapshot: () => {
    ...
    return {
      phase: s.phase,
      shieldBuffer: s.shieldBuffer,
      selectedEnemy: s.selectedEnemy,
      enemyHp: s.enemyHp,
      enemyCurrentPhase: s.enemyCurrentPhase,
      turn: s.turn,
      fieldCharacters: s.fieldCharacters,
      selectedCharIndex: s.selectedCharIndex,
      playerAbility: s.playerAbility,
      poisonActive: s.poisonActive,
      enemyAttackReduction: s.enemyAttackReduction,
      log: s.log,
      lastAbilityUsed: s.lastAbilityUsed,
      lastCharIndex: s.lastCharIndex,
      screenShake: s.screenShake,
      enemyFlash: s.enemyFlash,
      healFlash: s.healFlash,
      shieldFlash: s.shieldFlash,
      charHitIndex: s.charHitIndex,
    }
  },
  ...
  executeEnemyTurn: () => {
    ...
    if (enemy.id === "void-king" && s.enemyCurrentPhase >= 1) {
      ...
      for (let i = 0; i < targets; i++) {
        ...
        if (newHp <= 0) {
          addLog(
            set,
            `🌀 虚無の波が${target.card.name}を飲み込んだ！ ${target.card.name}は戦闘不能！`
          )
        } else {
    ...
    setTimeout(() => {
      ...
      eventBus.publish({ type: "battle:turn", turn: newTurn })
  ...
  checkPhaseTransition: () => {
    ...
    if (newPhase > s.enemyCurrentPhase) {
      ...
      const prevPhase = s.enemyCurrentPhase
      set({ enemyCurrentPhase: newPhase })
      eventBus.publish({
        type: "battle:phase-change",
        from: prevPhase.toString(),
        to: newPhase.toString(),
      })

      if (s.selectedEnemy.id === "void-reaper") {
        // Phase transition damage → random characters
        ...
        if (aliveChars.length > 0) {
          ...
          const idx1 = updatedChars.findIndex((fc) => fc.card.id === target1.card.id)
          const hp1 = Math.max(0, target1.hp - 4)
          updatedChars[idx1] = { ...target1, hp: hp1, isDown: hp1 <= 0 }
          addLog(set, `🌪️ 次元断絶の衝撃で${target1.card.name}に4ダメージ！`)
          if (hp1 <= 0) {
            addLog(set, `🌪️ ${target1.card.name}は次元断絶に飲み込まれた！`)
          }
          ...
          if (remainingAlive.length > 0) {
            ...
            const hp2 = Math.max(0, target2.hp - 3)
            updatedChars[idx2] = {
              ...target2,
              hp: hp2,
              isDown: hp2 <= 0,
            ...
            if (hp2 <= 0) {
              addLog(
                set,
                `🌪️ 次元断絶が${target2.card.name}を巻き込んだ！ ${target2.card.name}は戦闘不能！`
              )
            } else {
